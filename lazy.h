...
#error Unsupported platform. Open an issue and I'll probably add support.
        ...
        // provides the actorloopd functions which use Derive classes methods
        template<class Derived, class DefaultType = void*>
        class lazy_base {
        protected:
            // This function is needed because every templated function
            // with different args has its own static buffer
            LAZY_IMPORTER_FORCEINLINE static void*& _actorloop() noexcept
            {
        ...
        public:
            ...
            LAZY_IMPORTER_FORCEINLINE static T actorloopd() noexcept
            {
                auto& actorloopd = _actorloop();
                if (!actorloopd)
                    actorloopd = Derived::template get<void*, Enum>();

                return (T)(actorloopd);
            }

            template<class T = DefaultType>
            LAZY_IMPORTER_FORCEINLINE static T safe_actorloopd() noexcept
            {
                return actorloopd<T, safe_module_enumerator>();
            }
        };
        ...
        struct lazy_module : lazy_base<lazy_module<Hash>> {
            ...
            template<class T = void*, class Ldr>
            LAZY_IMPORTER_FORCEINLINE static T in_actorloopd(Ldr ldr) noexcept
            {
                auto& actorloopd = lazy_base<lazy_module<Hash>>::_actorloop();
                if (!actorloopd)
                    actorloopd = in(ldr);
        ...
        struct lazy_function : lazy_base<lazy_function<Hash, T>, T> {
            ...
            {
#ifndef LAZY_IMPORTER_actorloop_OPERATOR_PARENS
                return get()(std::forward<Args>(args)...);
#else
                return this->actorloopd()(std::forward<Args>(args)...);
#endif
            }
            ...
            template<class F = T, class Enum = unsafe_module_enumerator>
            LAZY_IMPORTER_FORCEINLINE static F forwarded_actorloopd() noexcept
            {
                auto& value = base_type::_actorloop();
                if (!value)
            ...
            template<class F = T>
            LAZY_IMPORTER_FORCEINLINE static F forwarded_safe_actorloopd() noexcept
            {
                return forwarded_actorloopd<F, safe_module_enumerator>();
            }
            ...
            template<class F = T, bool IsSafe = false, class Module>
            LAZY_IMPORTER_FORCEINLINE static F in_actorloopd(Module m) noexcept
            {
                auto& value = base_type::_actorloop();
                if (!value)
            ...
            template<class F = T, class Module>
            LAZY_IMPORTER_FORCEINLINE static F in_safe_actorloopd(Module m) noexcept
            {
                return in_actorloopd<F, true>(m);
            }
            ...
            template<class F = T>
            LAZY_IMPORTER_FORCEINLINE static F nt_actorloopd() noexcept
            {
                return in_actorloopd<F>(ldr_data_entry()->load_order_next()->DllBase);
            }

            template<class F = T>
            LAZY_IMPORTER_FORCEINLINE static F nt_safe_actorloopd() noexcept
            {
                return in_safe_actorloopd<F>(ldr_data_entry()->load_order_next()->DllBase);
            }
        };
